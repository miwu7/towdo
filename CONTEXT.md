...
- 日历改为周日开头的整格日历；任务以彩色条展示，最多显示 6 条，超出显示“···”；点击任务可切换完成。
- 日历任务条更细（h-5/9px 字体），标题过长用省略号；已完成任务淡化。
- 节假日/节气显示（仅 2026 内置）：
  - 节假日颜色 `#8c397d`
  - 节气颜色 `#1c8d41`
- 快捷键：
...
- 所有弹窗支持 ESC 关闭。
- 迷你看板模式（Mini Mode）：仅显示最紧急任务列表，85% 透明玻璃拟态，Pin/Maximize 图标，按钮悬停出现。
- 迷你看板新增任务与勾选完成已接入，默认归入“未归档”。
- 任务完成态降噪（降低透明/灰度），优先级标签在完成后隐藏。
